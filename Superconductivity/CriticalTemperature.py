...
def plotErrorBars(rangelow,rangehigh):
    plt.plot(AllanBradleyToTemp(AllanBradley[rangelow:rangehigh],True)[0],voltage[rangelow:rangehigh])
...
plt.plot(AllanBradleyToTemp(AllanBradley[1300:1500],True)[0],voltage[1300:1500],label='Messwerte')
...
plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f"{x * 100000:.0f}"))
...
plt.ylabel('Spannnung $U_{Probe}\,[10\, \mu V]$')
...
plt.legend()
plt.savefig("out/TCrit.png", dpi=300, bbox_inches='tight', pad_inches=0.05)
plt.show()
